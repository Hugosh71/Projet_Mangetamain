...
from mangetamain.preprocessing.feature.nutrition.strategies import (
    NutritionCleaning,
    NutritionPreprocessing,
)


def test_nutrition_analyser_stub() -> None:
...
def test_nutrition_strategies_stub() -> None:
    recipes = pd.DataFrame([{"id": 1}])
    interactions = pd.DataFrame([{"recipe_id": 1}])
    cleaner = NutritionCleaning()
    pre = NutritionPreprocessing()
