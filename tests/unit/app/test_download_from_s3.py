...
def test_download_from_s3_falls_back_to_stub(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    out = download_from_s3(bucket="mangetamain", key="recipes_merged.csv.gz", dest_dir=tmp_path)
    assert out.exists()
